...
console.log(
  `capitalize("${testString1}"): "${(0, stringUtils_1.capitalize)(
    testString1
  )}"`
)
console.log(
  `reverseString("${testString1}"): "${(0, stringUtils_1.reverseString)(
    testString1
  )}"`
)
console.log(`Исходная строка: "${testString2}"`)
console.log(
  `capitalize("${testString2}"): "${(0, stringUtils_1.capitalize)(
    testString2
  )}"`
)
console.log(
  `reverseString("${testString2}"): "${(0, stringUtils_1.reverseString)(
    testString2
  )}"`
)
console.log(`Исходная строка: "${testString3}"`)
console.log(
  `capitalize("${testString3}"): "${(0, stringUtils_1.capitalize)(
    testString3
  )}"`
)
console.log(
  `reverseString("${testString3}"): "${(0, stringUtils_1.reverseString)(
    testString3
  )}"`
)
// Задание 2
...
const annualRate = 12
const loanTerm = 24
const monthlyPayment = finance_1.Finance.LoanCalculator.calculateMonthlyPayment(
  loanAmount,
  annualRate,
  loanTerm
)
...
const admin = new userManagement_1.UserManagement.Admin.AdminUser(
  'Иван Иванов',
  'ivan@example.com',
  false
)
