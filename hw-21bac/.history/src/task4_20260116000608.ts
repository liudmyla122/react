...
class SavingsAccount extends Account {
  ...
  deposit(amount: number): void {
    if (amount > 0) {
      this.balance += amount
      console.log(
        `Внесено на сберегательный счет: ${amount}. Баланс: ${this.balance}`
      )
    }
  }
  ...
  withdraw(amount: number): void {
    if (amount > 0 && amount <= this.balance) {
      this.balance -= amount
      console.log(
        `Снято со сберегательного счета: ${amount}. Баланс: ${this.balance}`
      )
    } else {
  ...
  withdraw(amount: number): void {
    if (amount > 0) {
      ...
      if (totalAmount <= this.balance) {
        ...
        console.log(
          `Снято с текущего счета: ${amount}, комиссия: ${commission.toFixed(
            2
...
console.log('Сберегательный счет')
savingsAccount.deposit(1000)
savingsAccount.withdraw(200)
savingsAccount.addInterest()

const checkingAccount = new CheckingAccount(0.01)
console.log('Текущий счет')
