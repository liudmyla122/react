...
app.get('/', (req, res) => {
  res.status(200).send('Hello, World! Express Server is Ready.')
})
...
app.post('/', (req, res) => {
  ...
  if (!data || Object.keys(data).length === 0) {
    return res
      .status(400)
      .json({
        error: 'Некорректный запрос. Тело запроса не может быть пустым.',
      })
  }
  res
    .status(201)
    .json({ message: 'Данные успешно обработаны', receivedData: data })
})
